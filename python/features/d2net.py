...
MAX_KPTS = 1000
class d2net(DetectorAndDescriptor):
    def __init__(self, model_file=os.path.join(dirname, './d2net_misc/d2_tf.pth'), use_relu=True):
        super(
            d2net,
            self).__init__(
                name='D2Net',
    ...
    def detect_feature(self, image):
        ...
        resized_image, fact_i, fact_j = resize_image(image)
        ...
        order = np.argsort(scores)[::-1][:nb_kpts]

        keypoints[:, 0] *= fact_i
        keypoints[:, 1] *= fact_j
        keypoints = keypoints[:, [1, 0, 2]]
        keypoints = keypoints[order,:2]
...
class D2Net(nn.Module):
    def __init__(self, model_file=None, use_relu=True, use_cuda=True):
        ...
        if model_file is not None:
            ...
            if not use_cuda:
                location = 'cpu'
            self.load_state_dict(torch.load(model_file, map_location=location)['model'])
